...
# --- 新增辅助函数 ---
def update_ema(ema_model, training_model, momentum=0.999):
    """
    使用动量（momentum）更新 EMA 模型的权重。
    ema_model 的权重 = momentum * ema_model.old_weights + (1 - momentum) * training_model.new_weights
    """
    with torch.no_grad():
        for ema_param, param in zip(ema_model.parameters(), training_model.parameters()):
            ema_param.data.mul_(momentum).add_(param.data, alpha=1 - momentum)

# --- VirtualBatchNorm1d 类 (保持不变) ---
